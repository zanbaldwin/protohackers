use crate::{
    io::ClientInput, BufferMatch, PlateNumber, MESSAGE_TYPE_AM_CAMERA, MESSAGE_TYPE_AM_DISPATCHER,
    MESSAGE_TYPE_PLATE, MESSAGE_TYPE_WANT_HEARTBEAT,
};
use nom::{
    branch::alt,
    bytes::streaming::tag,
...
fn nom_plate(input: &[u8]) -> IResult<&[u8], ClientInput> {
    map(
        tuple((tag([MESSAGE_TYPE_PLATE]), nom_str, be_u32)),
        |(_, plate, timestamp)| ClientInput::Plate(plate, timestamp),
    )(input)
}

fn nom_heartbeat(input: &[u8]) -> IResult<&[u8], ClientInput> {
    map(
        tuple((tag([MESSAGE_TYPE_WANT_HEARTBEAT]), be_u32)),
        |(_, interval)| ClientInput::WantHeartbeat(interval),
    )(input)
}

fn nom_camera(input: &[u8]) -> IResult<&[u8], ClientInput> {
    map(
        tuple((tag([MESSAGE_TYPE_AM_CAMERA]), be_u16, be_u16, be_u16)),
        |(_, road, mile, limit)| ClientInput::IAmCamera(road, mile, limit),
    )(input)
}

fn nom_dispatcher(input: &[u8]) -> IResult<&[u8], ClientInput> {
    map(
        tuple((tag([MESSAGE_TYPE_AM_DISPATCHER]), nom_u16_arr)),
        |(_, roads)| ClientInput::IAmDispatcher(roads),
    )(input)
}

pub(crate) fn nom(input: &[u8]) -> BufferMatch {
    match alt((nom_plate, nom_camera, nom_dispatcher, nom_heartbeat))(input) {
        Ok((remainder, client_message)) => {
            Ok(Some((client_message, input.len() - remainder.len())))
        }
        Err(err) => match err {
...
mod tests {
    ...
    #[test]
    fn test_invalid() {
        assert_eq!(Err(()), nom(&[0u8]));
    }

    #[test]
    fn test_camera_incomplete() {
        assert_eq!(Ok(None), nom(&[0x80u8, 0x03, 0x11, 0x0c, 0x9d]));
    }
    ...
    fn test_dispatcher_exact() {
        assert_eq!(
            Ok(Some((ClientInput::IAmDispatcher(vec![785]), 4))),
            nom(&[0x81u8, 0x01, 0x03, 0x11])
        );
    }
    ...
    fn test_dispatcher_single_overflow() {
        assert_eq!(
            Ok(Some((ClientInput::IAmDispatcher(vec![785]), 4))),
            nom(&[0x81u8, 0x01, 0x03, 0x11, 0xab])
        );
    }
